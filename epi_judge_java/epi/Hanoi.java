...
public class Hanoi {
  ...
  public static List<List<Integer>> computeTowerHanoi(int numRings) {
      ...
      computeTowerHanoi(numRings, 0, 1, 2, result);
      return result;
  }

  private static void computeTowerHanoi(int n, int src, int temp, int dest, List<List<Integer>> result) {
        if(n==1) {
            result.add(Arrays.asList(src, dest));
            return;
        }
        computeTowerHanoi(n-1, src, dest, temp, result);
        result.add(Arrays.asList(src, dest));
        computeTowerHanoi(n-1, temp, src, dest, result);
  }
